...
class Renderer {
  ...
  drawEntity(
    ...
    if (!this.validateEntity(entity)) {
      console.warn(`跳过无效实体绘制（ID: ${this.getEntityId(entity)}）`);
      return;
    }
  ...
  /**
   * 获取 2D 渲染上下文（供 Entity.render 等扩展绘制使用）
   */
   ...
   * @returns 是否有效
   */
  private validateEntity(entity: unknown): entity is RenderableEntity | PlayerStateType {
    if (typeof entity !== 'object' || entity === null) {
      return false;
    }

    const { location, size } = entity as { location?: unknown; size?: unknown };
