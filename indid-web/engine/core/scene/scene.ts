...
export class Scene {
  ...
  /**
   * 添加实体到场景（代理实体池的add方法）
   * @param entity 游戏实体（Entity 基类及其子类）
   */
  addEntity(entity: Entity): void {
    if (!entity.id) throw new Error(`[Scene] 实体ID不能为空`);
    this.entityPool.add(entity);
    console.log(`[Scene-${this.id}] 实体${entity.id}已添加到场景`);
  }
  ...
  removeEntity(entityId: string): void {
    this.entityPool.remove(entityId);
    console.log(`[Scene-${this.id}] 实体${entityId}已从场景移除`);
  }
  ...
  getEntity(entityId: string): Entity | undefined {
    return this.entityPool.get(entityId);
  }
  ...
  destroy(): void {
    ...
    this.entityPool.destroy((entity) => {
      console.log(`[Scene-${this.id}] 实体${entity.id}已销毁`);
  ...
  private update(deltaTime: number): void {
    ...
    // 1. 遍历场景内所有实体，调用实体自身的 update 方法
    this.entityPool.forEach((entity) => {
      entity.update(deltaTime);
    });

    // 2. 实体间 AABB 碰撞检测与简单分离响应
    this.detectCollisions();

    // 3. 执行自定义场景更新逻辑（如关卡倒计时、场景事件）
    this.hooks.onUpdate?.(this, deltaTime);
  }

  /**
   * 遍历实体池，检测并响应 AABB 碰撞
   * 对可移动实体（Player）施加最小分离位移
   */
  private detectCollisions(): void {
    const entities = this.entityPool.getAll();
    ...
    for (let i = 0; i < entities.length; i++) {
      for (let j = i + 1; j < entities.length; j++) {
        ...
        const bMovable = this.isMovable(b);

        if (aMovable && !bMovable) {
          const [x, y] = a.getLocation();
          a.setLocation(x + correction.x, y + correction.y);
        } else if (bMovable && !aMovable) {
          const [x, y] = b.getLocation();
          b.setLocation(x - correction.x, y - correction.y);
        } else if (aMovable && bMovable) {
          const [ax, ay] = a.getLocation();
          a.setLocation(ax + correction.x / 2, ay + correction.y / 2);
          const [bx, by] = b.getLocation();
          b.setLocation(bx - correction.x / 2, by - correction.y / 2);
        }
      }
    }
  }
