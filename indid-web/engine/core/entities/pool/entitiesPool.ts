...
type EntityPoolHooks = {
  ...
  onClear?: () => void;
  /** 实体被回收至空闲池时触发（对象复用） */
  onRecycle?: (entity: Entity) => void;
  /** 实体从空闲池被复用时时触发（对象复用） */
  onReuse?: (entity: Entity) => void;
};
 ...
 * 实体池模块（纯托管+对象复用版）
 * 核心职责：
 * 1. 纯托管 Entity 实例的增删改查与生命周期管理
 * 2. 实现 Entity 实例的复用，减少GC开销
 * 3. 提供高效的实体筛选、遍历能力
 * 不介入实体内部属性修改，完全由实体自身方法负责
 */
class EntityPool {
  // 核心：托管实体的Map（id -> 实例），保证ID查询O(1)
  private readonly entities = new Map<string, Entity>();
  // 性能优化：实体数组缓存，避免每次getAll都执行Array.from
  private readonly _entityCache: Entity[] = [];
  // 对象复用：空闲实体池，存储已回收的 Entity 实例
  private readonly freePool: Entity[] = [];
  // 生命周期钩子
   ...
   * @returns 成功添加的实体数量
   */
  batchAdd(entities: Entity[], errorStrategy: BatchErrorStrategy = BatchErrorStrategy.ABORT): number {
    // 1. 入参数组校验
    if (!Array.isArray(entities)) {
      throw new Error(`[EntityPool] 批量添加失败：入参必须是 Entity 实例数组`);
    }
   ...
   */
  batchGet(ids: string[]): (Entity | undefined)[] {
   ...
   * @returns Player实例
   */
  getOrCreate(createFn: () => Entity, resetFn: (entity: Entity) => void): Entity {
    // 1. 优先从空闲池取复用实体
   ...
   */
  clearFreePool(destroyFn?: (entity: Entity) => void): void {
    if (destroyFn) {
   ...
   */
  filterByShape(shape: PlayerShape): Entity[] {
    return this.filter(entity => entity instanceof Player && entity.shape === shape);
  }
   ...
   */
  filterByLocationRange(xRange: NumericRange, yRange: NumericRange): Entity[] {
    // 1. 校验坐标范围的合法性
   ...
   */
  filterByOpacity(opacityRange: NumericRange = [0, 1]): Entity[] {
    // 1. 校验透明度范围的合法性
    ...
    return this.filter(entity => {
      const opacity = entity instanceof Player ? (entity.opacity ?? 1) : 1;
   ...
   */
  forEach(callback: (entity: Entity, index: number, array: Entity[]) => void): void {
   ...
   * @param entity 待校验的实体
   */
  private _validateEntity(entity: unknown): asserts entity is Entity {
    if (!(entity instanceof Entity)) {
      throw new Error(`[EntityPool] 实体必须是 Entity 类的实例`);
    }
   ...
   */
  destroy(destroyFn?: (entity: Entity) => void): void {
    // 1. 清空托管池
    this.clear();
